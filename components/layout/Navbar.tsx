...
import { Search, ShoppingCart, User, Heart, Menu, X, ChevronDown, Instagram, Loader2 } from "lucide-react"
        ...
        const fetchData = async () => {
            try {
                const [catRes, setRes] = await Promise.all([
                    fetch('/api/categories?all=true&includeSubcategories=true'),
                    fetch('/api/settings')
                ])
                ...
                if (catResult.data) {
                    ...
                    if (catResult.data.length > 0) {
                        setActiveMegaCategory(catResult.data[0])
                    }
                }
                if (setResult.data) {
                ...
                <div className="max-w-[1280px] mx-auto text-[8px] md:text-[10px] font-sans  py-2 px-4 flex justify-between items-center tracking-[0.2em] uppercase">
                    ...
                    <div className="flex gap-4 items-center">
                        {settings.instagram_url && (
                            <Link href={settings.instagram_url} target="_blank" className="hover:text-accent transition-colors">
                                <Instagram className="w-3.5 h-3.5" />
                            </Link>
                        )}
                        {settings.whatsapp_number && (
                            <Link href={`https://wa.me/${settings.whatsapp_number.replace(/\D/g, '')}`} target="_blank" className="hover:text-accent transition-colors">
